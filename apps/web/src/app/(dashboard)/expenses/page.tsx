...
export default function ExpensesPage() {
  ...
  const [longPressExpense, setLongPressExpense] = useState<any | null>(null);
          ...
          <div className="grid gap-4 md:hidden">
            {expenses.map((e: any) => (
              <MobileExpenseCard
                key={e.id}
                expense={e}
                onLongPress={() => setLongPressExpense(e)}
                onDelete={() => handleDelete(e.id, e.category, Number(e.amount || 0))}
              />
            ))}
          </div>
