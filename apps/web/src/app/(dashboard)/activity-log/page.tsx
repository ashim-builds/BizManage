  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-5">
        <div>
          <div className="flex items-center gap-3">
            <h1 className="text-xl sm:text-2xl font-bold text-slate-900 flex items-center gap-2.5">
              <ShieldCheck className="w-6 h-6 sm:w-7 sm:h-7 text-purple-600" /> Business Activity Log
            </h1>
          ...
          <p className="text-xs text-slate-500 mt-1">
            Complete operational activity history for sales, purchases, products, inventory, and financial entries.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2.5">
          <button
            type="button"
            onClick={() => refetch()}
            className="p-2.5 rounded-xl bg-white border border-slate-200 text-slate-600 hover:text-slate-900 hover:bg-slate-50 transition-all shadow-sm"
            title="Refresh logs"
          >
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-purple-600 hover:bg-purple-700 disabled:opacity-50 text-white text-xs font-semibold shadow-sm transition-all min-h-[44px]"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="p-4 sm:p-5 rounded-2xl bg-white border border-slate-200 flex items-center gap-4 shadow-sm">
          ...
          <div>
            <p className="text-xs font-medium text-slate-500">Total Events Logged</p>
            <h3 className="text-xl font-bold text-slate-900 mt-0.5">{summary.totalEvents.toLocaleString()}</h3>
          </div>
        </div>
        ...
        <div className="p-4 sm:p-5 rounded-2xl bg-white border border-slate-200 flex items-center gap-4 shadow-sm">
          ...
          <div>
            <p className="text-xs font-medium text-slate-500">Sales Invoices & Returns</p>
            <h3 className="text-xl font-bold text-emerald-600 mt-0.5">{summary.salesCount.toLocaleString()}</h3>
          </div>
        </div>
        ...
        <div className="p-4 sm:p-5 rounded-2xl bg-white border border-slate-200 flex items-center gap-4 shadow-sm">
          ...
          <div>
            <p className="text-xs font-medium text-slate-500">Purchases & Inwards</p>
            <h3 className="text-xl font-bold text-blue-600 mt-0.5">{summary.purchasesCount.toLocaleString()}</h3>
          </div>
        </div>
        ...
        <div className="p-4 sm:p-5 rounded-2xl bg-white border border-slate-200 flex items-center gap-4 shadow-sm">
          ...
          <div>
            <p className="text-xs font-medium text-slate-500">Product & Stock Changes</p>
            <h3 className="text-xl font-bold text-amber-600 mt-0.5">{summary.inventoryCount.toLocaleString()}</h3>
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto pb-1 max-w-full text-xs">
            {[
              ...
              <button
                ...
                className={`px-3.5 py-2 rounded-xl font-semibold transition-all shrink-0 min-h-[38px] ${
          ...
          <div className="relative w-full sm:w-64 shrink-0">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 text-xs placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-600 transition-all min-h-[44px]"
            />
          </div>
        </div>
      </div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid gap-3 md:hidden">
            {logs.map((log) => {
              ...
              return (
                <div key={log.id} className="p-4 rounded-2xl bg-white border border-slate-200 shadow-sm space-y-3">
                  <div className="flex items-center justify-between gap-2">
                    <span
                      className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider border flex items-center gap-1 w-max ${getModuleBadgeStyle(
                        log.module
                      )}`}
                    >
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-slate-100">
                    <div className="flex items-center gap-2 min-w-0">
                      <div className="w-6 h-6 rounded-full bg-slate-100 border border-slate-200 text-slate-700 flex items-center justify-center font-bold text-[10px] shrink-0">
                        {log.user?.name ? log.user.name.charAt(0).toUpperCase() : <User className="w-3 h-3" />}
                      </div>
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-purple-50 text-purple-700 hover:bg-purple-100 text-xs font-semibold transition-colors min-h-[36px]"
                    >
          ...
          <div className="hidden md:block border border-slate-200 rounded-2xl overflow-x-auto bg-white shadow-sm">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-2 text-xs text-slate-600">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="px-4 py-2 rounded-xl bg-white border border-slate-200 disabled:opacity-40 text-slate-700 hover:bg-slate-50 font-semibold shadow-sm transition-all min-h-[40px]"
                >
                  Previous
                </button>
                <button
