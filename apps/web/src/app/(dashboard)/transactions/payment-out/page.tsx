...
export default function PaymentOutPage() {
  ...
  const renderMobileCard = (p: any) => (
    <div className="bg-white rounded-2xl border border-slate-200/90 p-4 shadow-2xs space-y-3">
      {/* Top Header: Supplier info + Amount */}
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="text-right shrink-0">
          <span className={`font-mono font-black text-sm block ${p.status === 'VOIDED' ? 'line-through text-slate-400' : 'text-rose-600'}`}>
            - Rs. {Number(p.amount || 0).toLocaleString()}
          </span>
          {p.status === 'VOIDED' && (
            <span className="inline-block mt-0.5 px-1.5 py-0.5 rounded-md bg-slate-100 border border-slate-200 text-slate-500 font-bold text-[9px] uppercase">
              Voided
            </span>
          )}
        </div>
    ...
    <div className="max-w-7xl mx-auto space-y-6 px-3 sm:px-6 py-4">
      ...
      <div className="space-y-2 md:hidden">
        ...
        <div className="bg-rose-50/80 border border-rose-200/90 rounded-2xl p-3 sm:p-3.5 flex items-center justify-between shadow-2xs">
          <div className="min-w-0 pr-2">
            <p className="text-[11px] font-bold text-rose-700 uppercase tracking-wide">Total Vendor Payouts</p>
            ...
            <p className="text-[10px] text-slate-500 font-medium mt-0.5">
              {summary?.totalPaymentsCount || 0} Payouts settled
            </p>
          </div>
        ...
        {/* Breakdown Row (2 Columns): Total Vouchers + Today's Payouts */}
        <div className="grid grid-cols-2 gap-2">
          ...
          <div className="bg-blue-50/70 border border-blue-200/80 rounded-2xl p-2.5 sm:p-3 flex items-center justify-between shadow-2xs">
            <div className="min-w-0 pr-1">
              ...
              <p className="text-[9px] text-blue-600/80 font-semibold mt-0.5 truncate">Vouchers</p>
            </div>
          ...
          <div className="bg-amber-50/70 border border-amber-200/80 rounded-2xl p-2.5 sm:p-3 flex items-center justify-between shadow-2xs">
            <div className="min-w-0 pr-1">
              <p className="text-[11px] font-bold text-amber-700 truncate">Today's Paid</p>
      ...
      <div className="hidden md:grid md:grid-cols-3 gap-4 lg:gap-6">
        <div className="p-5 lg:p-6 rounded-2xl bg-white border border-slate-200/90 shadow-xs flex items-center justify-between">
          <div>
            ...
            <p className="text-[11px] text-slate-400 mt-1">Payment vouchers recorded</p>
          </div>
        ...
        <div className="p-5 lg:p-6 rounded-2xl bg-white border border-slate-200/90 shadow-xs flex items-center justify-between">
          <div>
            <p className="text-xs font-bold text-slate-500 uppercase tracking-wider">Total Vendor Payouts</p>
            <h3 className="text-2xl font-black text-rose-600 mt-1 font-mono">
              Rs. {summaryLoading ? '...' : (summary?.totalPaymentsAmount || 0).toLocaleString()}
            </h3>
            <p className="text-[11px] text-rose-600/80 mt-1 font-medium">Supplier bills settled</p>
          </div>
        ...
        <div className="p-5 lg:p-6 rounded-2xl bg-white border border-slate-200/90 shadow-xs flex items-center justify-between">
          <div>
            <p className="text-xs font-bold text-slate-500 uppercase tracking-wider">Today's Payouts</p>
            <h3 className="text-2xl font-black text-amber-600 mt-1 font-mono">
              Rs. {summaryLoading ? '...' : (summary?.todayPaymentsAmount || 0).toLocaleString()}
            </h3>
            <p className="text-[11px] text-slate-400 mt-1">Paid out today</p>
          </div>
          ...
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4 md:p-6 animate-in fade-in">
            <div className="w-full h-[100dvh] sm:h-auto sm:max-h-[92vh] sm:max-w-lg rounded-t-3xl sm:rounded-3xl flex flex-col bg-white overflow-hidden shadow-2xl animate-in zoom-in-95">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <h3 className="text-base sm:text-lg font-black text-slate-900 tracking-tight">Record Payment Out</h3>
                    ...
                    <p className="text-xs text-slate-500 font-medium">Record vendor payouts & clear supplier dues</p>
                  </div>
                </div>
                <button
                  type="button"
              ...
              <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4">
                <form id="payment-out-form" onSubmit={form.handleSubmit(onSubmitInitiate)} className="space-y-4">
                  <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                    ...
                    <div>
                      <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">Deduct From Account</label>
                      <select
              ...
              <div className="p-4 sm:p-5 bg-slate-50 border-t border-slate-200 flex items-center justify-end gap-3 shrink-0">
                ...
                >
                  {createPaymentOut.isPending ? 'Recording...' : 'Record Payout'}
                </button>
              </div>
            </div>
          </div>
      ...
      <div className="fixed bottom-20 left-1/2 -translate-x-1/2 z-30 lg:hidden pointer-events-auto">
        ...
        >
          ...
          <span>Add Payment Out (भुक्तानी)</span>
        </button>
      </div>
    </div>
  );
