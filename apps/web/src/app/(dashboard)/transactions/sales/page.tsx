    ...
    for (const [itemId, totalQty] of qtyMap.entries()) {
      ...
      if (item && item.type === 'PRODUCT') {
        ...
        if (stock <= 0) {
          ...
          errors.push(
            `"${item.name}" exceeds available inventory (Available: ${stock} ${item.unit}, Total Requested: ${totalQty} ${item.unit}).`
          );
        }
      }
    }
  ...
  const handleSaleSaveRequest = (data: CreateSaleInput) => {
    ...
    for (const [itemId, totalQty] of itemQtyMap.entries()) {
      ...
      if (sel && sel.type === 'PRODUCT') {
        ...
        if (totalQty > stock) {
          toast.error(
            `Insufficient stock for "${sel.name}". Available: ${stock} ${sel.unit}, Total requested in bill: ${totalQty} ${sel.unit}.`
          );
          return;
        }
      }
      ...
      <div className="space-y-2 md:hidden">
        ...
        <div className="bg-blue-50/80 border border-blue-200/90 rounded-2xl p-3 sm:p-3.5 flex items-center justify-between shadow-2xs">
          <div className="min-w-0 pr-2">
            <p className="text-[11px] font-bold text-blue-700 uppercase tracking-wide">Total Sales Revenue</p>
        ...
        {/* Breakdown Row (2 Columns): Total Collected + To Receive */}
        <div className="grid grid-cols-2 gap-2">
          ...
          <div className="bg-emerald-50/70 border border-emerald-200/80 rounded-2xl p-2.5 sm:p-3 flex items-center justify-between shadow-2xs">
            <div className="min-w-0 pr-1">
              <p className="text-[11px] font-bold text-emerald-700 truncate">Total Collected</p>
              ...
              <p className="text-[9px] text-emerald-600/80 font-semibold mt-0.5 truncate">Paid / Settled</p>
            </div>
          ...
          <div className="bg-amber-50/70 border border-amber-200/80 rounded-2xl p-2.5 sm:p-3 flex items-center justify-between shadow-2xs">
            <div className="min-w-0 pr-1">
              <p className="text-[11px] font-bold text-amber-700 truncate">To Receive</p>
              ...
              <p className="text-[9px] text-amber-700/80 font-semibold mt-0.5 truncate">
                {summary?.unpaidCount || 0} Due Invoices
              </p>
            </div>
      ...
      <div className="hidden md:grid md:grid-cols-3 gap-4 lg:gap-6">
        <div className="p-5 lg:p-6 rounded-2xl bg-white border border-slate-200/90 shadow-xs flex items-center justify-between">
          <div>
            ...
            <p className="text-[11px] text-slate-400 mt-1">{summary?.totalSalesCount || 0} Invoices issued</p>
          </div>
        ...
        <div className="p-5 lg:p-6 rounded-2xl bg-white border border-slate-200/90 shadow-xs flex items-center justify-between">
          <div>
            ...
            <p className="text-[11px] text-slate-400 mt-1">{summary?.unpaidCount || 0} Outstanding due invoices</p>
          </div>
      ...
      <div className="space-y-2.5 md:space-y-0 md:flex md:items-center md:justify-between md:gap-3 p-3 sm:p-4 rounded-2xl bg-white border border-slate-200/90 shadow-2xs">
        ...
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-9 pr-8 py-2.5 rounded-2xl bg-slate-50 md:bg-white border border-slate-200 text-xs font-semibold text-slate-900 placeholder:text-slate-400 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all"
          />
          {search && (
            <button
              type="button"
        ...
        <div className="grid grid-cols-5 gap-1 w-full md:w-auto md:flex md:items-center md:gap-1.5 text-[10px] sm:text-xs font-bold">
          ...
          <button
            ...
            className={`py-2 px-1 sm:px-3 sm:py-1.5 rounded-xl text-center transition-all cursor-pointer truncate ${
              selectedStatus === InvoiceStatus.PARTIAL
          ...
          <button
            ...
            className={`py-2 px-1 sm:px-3 sm:py-1.5 rounded-xl text-center transition-all cursor-pointer truncate ${
              selectedStatus === InvoiceStatus.RETURNED
                  ...
                  return (
                    <tr key={s.id} className="hover:bg-slate-50/80 transition-colors group">
                      <td className="px-5 py-4 font-semibold text-slate-900 border-r border-slate-100">
                        ...
                        <p className="text-[10px] text-slate-400 mt-0.5">{new Date(s.date).toLocaleDateString()}</p>
                      </td>
                      ...
                      <td className="px-5 py-4 text-right font-mono space-y-0.5 border-r border-slate-100">
                        ...
                        {due > 0 && (
                          <p className="text-amber-600 text-[11px] font-bold">Due: Rs. {due.toLocaleString()}</p>
                        )}
                      </td>
