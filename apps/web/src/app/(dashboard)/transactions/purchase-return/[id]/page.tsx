...
export default function PurchaseReturnDebitNotePage({ params }: { params: { id: string } }) {
  ...
  const documentItems = (purchaseReturn.items || []).map((it: any, index: number) => ({
    id: it.id,
    sn: index + 1,
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto font-sans">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5 font-medium">
              Issued on {new Date(purchaseReturn.date).toLocaleDateString('en-US', { day: 'numeric', month: 'short', year: 'numeric' })}
            </p>
          </div>
        </div>
      ...
      {/* UNIFIED MONOCHROME DEBIT NOTE PAPER VIEW */}
      <StandardMonochromeDocument
        documentTitle="DEBIT NOTE / PURCHASE RETURN"
        documentNumberLabel="Debit Note No"
        documentNumber={purchaseReturn.returnNumber}
        documentDate={purchaseReturn.date}
        referenceNumber={purchaseReturn.purchase?.billNumber || (purchaseReturn as any).originalBillNumber}
        referenceDate={purchaseReturn.purchase?.date}
        business={business}
