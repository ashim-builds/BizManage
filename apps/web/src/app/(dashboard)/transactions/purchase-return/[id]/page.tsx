...
import { usePurchaseReturn } from '@/services/purchaseService';
...
export default function PurchaseReturnDebitNotePage({ params }: { params: { id: string } }) {
  const { data: purchaseReturn, isLoading, isError, refetch } = usePurchaseReturn(params.id);
  const { data: business } = useCurrentBusiness();

  if (isLoading) return <LoadingState message="Loading debit note document..." />;
  if (isError || !purchaseReturn) return <ErrorState title="Failed to load debit note" onRetry={refetch} />;

  const subTotal = Number(purchaseReturn.subTotal || 0);
  const totalTax = Number(purchaseReturn.taxAmount || 0);
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto">
      ...
      <div className="flex items-center justify-between border-b border-slate-800 pb-5 print:hidden">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-bold text-white flex items-center gap-2">
              Debit Note <span className="font-mono text-blue-400">{purchaseReturn.returnNumber}</span>
            </h1>
            <p className="text-xs text-slate-400">Date: {new Date(purchaseReturn.date).toLocaleDateString()}</p>
          </div>
        </div>
        ...
        >
          <Printer className="w-4 h-4" /> Print Debit Note
        </button>
      </div>
        ...
        <div className="flex flex-wrap justify-between items-start border-b border-slate-800 print:border-slate-300 pb-6 gap-4">
          ...
          <div className="text-right">
            ...
            <p className="text-xs text-slate-400 print:text-slate-600 mt-1">
              Date: {new Date(purchaseReturn.date).toLocaleDateString()}
            </p>
          </div>
        </div>
        ...
        <div className="p-4 rounded-xl bg-slate-800/50 print:bg-slate-50 border border-slate-800 print:border-slate-200 grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
          <div>
            ...
            <p className="font-bold text-white print:text-slate-900 text-sm">
              {purchaseReturn.party?.name || 'Vendor'}
            </p>
            {purchaseReturn.party?.phone && (
              <p className="text-slate-300 print:text-slate-600 mt-0.5">Phone: {purchaseReturn.party.phone}</p>
            )}
            {purchaseReturn.party?.taxNumber && (
              <p className="text-slate-300 print:text-slate-600 font-mono">
                PAN/VAT: {purchaseReturn.party.taxNumber}
              </p>
            )}
          </div>
        </div>
        ...
        <div className="border border-slate-800 print:border-slate-200 rounded-xl overflow-hidden">
          <table className="w-full text-left text-xs min-w-[800px]">
            ...
            <tbody className="divide-y divide-slate-800/60 print:divide-slate-200 text-slate-300 print:text-slate-800">
              {(purchaseReturn.items || []).map((line: any) => (
                <tr key={line.id}>
