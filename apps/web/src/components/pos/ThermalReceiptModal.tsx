...
import { Printer, X, Receipt, Calculator } from 'lucide-react';
  ...
  return (
    <ModalPortal>
      ...
      <div className="fixed inset-0 bg-slate-950/85 backdrop-blur-md z-[100] flex items-center justify-center p-4 overflow-y-auto font-sans">
        <div className="w-full max-w-2xl bg-white border border-slate-300 rounded-3xl p-5 sm:p-6 shadow-2xl space-y-4 my-8 max-h-[95vh] flex flex-col text-slate-900">
          {/* Header */}
          <div className="flex flex-wrap items-center justify-between border-b border-slate-200 pb-3 gap-2 shrink-0">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 p-3 rounded-2xl bg-slate-50 border border-slate-200 text-xs shrink-0">
            <div>
              <label className="block text-[11px] font-bold text-slate-700 mb-1">Paper Roll Size</label>
              <div className="flex gap-1.5">
                <button
                  type="button"
                  onClick={() => setPaperWidth('80mm')}
                  className={`flex-1 py-1.5 px-2 rounded-xl font-bold transition-all text-xs border cursor-pointer ${
                    paperWidth === '80mm'
            ...
            <div>
              <label className="block text-[11px] font-bold text-slate-700 mb-1 flex items-center gap-1">
                <Calculator className="w-3.5 h-3.5 text-slate-700" /> Cash Tendered (Rs.)
              </label>
              <input
                type="text"
                inputMode="decimal"
                ...
                className="w-full px-3 py-1.5 rounded-xl bg-white border border-slate-300 text-slate-900 font-mono text-xs focus:outline-none focus:border-slate-800"
              />
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-6 bg-slate-100 rounded-2xl flex justify-center items-start border border-slate-200 shadow-inner max-h-[520px] min-h-[320px]">
            <div
              id="thermal-receipt-printable"
              ref={printAreaRef}
              className={`bg-white text-slate-950 p-4 sm:p-5 font-mono shadow-md border border-slate-300 rounded-none text-[11px] leading-tight space-y-3 transition-all w-full h-fit min-h-max shrink-0 select-text ${
            ...
            >
              ...
              <div className="text-center space-y-0.5 border-b border-dashed border-slate-400 pb-2.5">
                ...
                {showTax && taxNumber && (
                  <p className="text-[10px] text-slate-900 font-bold">{taxType}: {taxNumber}</p>
                )}
                <p className="text-[9px] text-slate-700 pt-0.5 font-bold uppercase">
              ...
              <div className="py-2 border-b border-dashed border-slate-400 text-[10px] space-y-1">
                ...
                <div className="pt-0.5 text-slate-700">
                  ...
                  {sale.party?.taxNumber && <span className="block text-[9px]">Party PAN/VAT: {sale.party.taxNumber}</span>}
                </div>
              </div>
                      ...
                      return (
                        <tr key={idx}>
                          <td className="py-1.5 font-bold text-slate-900 pr-1">
                            {line.item?.name || 'Item'}
                            {disc > 0 && (
                              <span className="block text-[8px] text-slate-600 font-medium">
              ...
              {/* Footer / Notes */}
              {showFooterNotes && (
                <div className="text-center text-[9px] text-slate-700 pt-2 border-t border-dashed border-slate-400 space-y-0.5">
          ...
          {/* Action Buttons */}
          <div className="flex justify-end gap-3 pt-2 border-t border-slate-200 shrink-0">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2 rounded-xl text-slate-700 hover:bg-slate-100 font-bold text-xs transition-colors cursor-pointer border border-slate-300"
            >
              Close
            </button>
            <button
              ...
              className="inline-flex items-center gap-2 px-5 py-2 rounded-xl bg-slate-900 hover:bg-black text-white font-bold text-xs transition-all shadow-md active:scale-95 cursor-pointer"
            >
