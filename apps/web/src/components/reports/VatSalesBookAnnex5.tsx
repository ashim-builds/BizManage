...
import { Download, Printer, FileSpreadsheet, ShieldCheck } from 'lucide-react';

interface VatSalesBookAnnex5Props {
  sales: any[];
  startDate?: string;
  endDate?: string;
  businessName?: string;
...
export function VatSalesBookAnnex5({
  sales = [],
  startDate,
  endDate,
  businessName = 'My Business',
  businessPan = '-',
}: VatSalesBookAnnex5Props) {
  ...
  const totals = useMemo(() => {
    ...
    sales.forEach((s) => {
      ...
      const vat = Number(s.taxAmount || 0);
      const isVat = s.isVatBill || vat > 0;
  ...
  const exportCsv = () => {
    ...
    const rows = sales.map((s, idx) => {
      const ad = new Date(s.date).toISOString().split('T')[0];
      const amount = Number(s.totalAmount || 0);
      const vat = Number(s.taxAmount || 0);
      const isVat = s.isVatBill || vat > 0;
    ...
    link.setAttribute(
      'download',
      `VAT_Sales_Book_Annex5_${new Date().toISOString().split('T')[0]}.csv`
    );
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="p-4 sm:p-6 rounded-2xl bg-white border border-slate-200 shadow-xs space-y-4 print:border-none print:p-0">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-200 pb-4 gap-3 print:border-black">
          <div>
            <div className="flex items-center gap-2">
              <span className="px-2.5 py-0.5 rounded-md bg-blue-50 text-blue-700 border border-blue-200 text-[10px] font-bold uppercase print:hidden">
                IRD Nepal Format
              </span>
              <h2 className="text-base sm:text-lg font-bold text-slate-900 print:text-black">
                अनुसूची-५ (बिक्री खाता) / Annex-5: Sales Book
              </h2>
            </div>
            <p className="text-xs text-slate-500 print:text-gray-700 mt-1">
              (नियम २३ को उपनियम (१) को खण्ड (ज) सँग सम्बन्धित) • Value Added Tax Rules, 2053
            </p>
          </div>
          ...
          <div className="flex items-center gap-2 print:hidden">
            ...
            >
              <Printer className="w-3.5 h-3.5" /> Print Annex-5
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
          ...
          <div className="p-3 rounded-xl bg-slate-50 border border-slate-200 print:bg-white">
            ...
            <p className="font-mono font-bold text-blue-600 text-sm mt-0.5">{businessPan}</p>
          </div>
          <div className="p-3 rounded-xl bg-slate-50 border border-slate-200 print:bg-white">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5 sm:gap-4 print:grid-cols-4">
        <div className="p-3.5 sm:p-4 rounded-2xl bg-white border border-slate-200 shadow-xs">
          <p className="text-[10px] sm:text-xs font-bold text-slate-500 uppercase truncate">जम्मा बिक्री (Total)</p>
          <h3 className="text-base sm:text-xl font-bold font-mono text-slate-900 mt-0.5 truncate">
            Rs. {totals.totalSales.toLocaleString(undefined, { minimumFractionDigits: 0 })}
          </h3>
        </div>
        <div className="p-3.5 sm:p-4 rounded-2xl bg-white border border-slate-200 shadow-xs">
          <p className="text-[10px] sm:text-xs font-bold text-blue-600 uppercase truncate">करयोग्य (Taxable)</p>
          <h3 className="text-base sm:text-xl font-bold font-mono text-blue-600 mt-0.5 truncate">
      ...
      <div className="grid gap-2.5 md:hidden">
        {sales.length === 0 ? (
          ...
          sales.map((s, idx) => {
            ...
            return (
              <div key={s.id || idx} className="p-3.5 bg-white border border-slate-200 rounded-2xl space-y-2.5 shadow-2xs text-xs">
                <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                  <div>
                    <span className="font-mono font-bold text-blue-600 text-xs">{s.invoiceNumber}</span>
                    <strong className="block text-slate-900 font-semibold">{s.party?.name || 'Walk-in Customer'}</strong>
                  </div>
                ...
                <div className="grid grid-cols-3 gap-1.5 bg-slate-50 p-2 rounded-xl border border-slate-200 text-[11px]">
                  ...
                  <div>
                    <span className="text-[10px] text-blue-600 block">Taxable:</span>
                    <strong className="font-mono text-blue-600">Rs. {taxable.toLocaleString()}</strong>
                  </div>
                  <div>
      ...
      {/* DESKTOP OFFICIAL ANNEX-5 TABLE (>= md) */}
        ...
        <table className="w-full text-left text-xs min-w-[850px]">
          <thead className="bg-slate-50 text-slate-700 font-bold border-b border-slate-200 print:border-black">
            <tr>
              ...
              <th className="px-4 py-3">बीजक नं. (Invoice)</th>
              <th className="px-4 py-3">खरिदकर्ताको नाम (Buyer)</th>
              <th className="px-4 py-3">स्थायी लेखा नं. (PAN)</th>
              <th className="px-4 py-3 text-right">जम्मा बिक्री (Total)</th>
            ...
            {sales.length === 0 ? (
              <tr>
                <td colSpan={9} className="text-center py-8 text-slate-400">
                  No sales invoices recorded for the selected period.
                </td>
              </tr>
            ) : (
              sales.map((s, idx) => {
                const ad = new Date(s.date).toLocaleDateString('en-US', { day: 'numeric', month: 'short', year: 'numeric' });
                const amount = Number(s.totalAmount || 0);
                const vat = Number(s.taxAmount || 0);
                const isVat = s.isVatBill || vat > 0;
                const taxable = isVat ? amount - vat : 0;
